            ...
            for (let y: number = 0; y < Chunk.chunkSize; y++) {
                ...
                for (let z: number = 0; z < Chunk.chunkSize; z++) {
                    ...
                    if (diffCount > 0 && (diff = this.save.getDiff(subChunkPos))) {
                        ...
                        if (!world.oldWorldGen) {
                            this.blocks[x][y].push(world.getBlockToGenerateAtFromChunk(this.getBlockPos(subChunkPos), bData[x][z]));
                        } else {
                            this.blocks[x][y].push(world.getBlockToGenerateAtOld(this.getBlockPos(subChunkPos)));
                        }
                    }
                }
            }
